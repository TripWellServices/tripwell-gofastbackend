...
const TripExtra = require("../../models/TripWell/TripExtra");

class TripExtraService {
  // 🔍 Validate what data is missing for a user and store in TripExtra model
  async validateUserData(firebaseId) {
    try {
      // 1. Check if user exists
      const user = await TripWellUser.findOne({ firebaseId });
      if (!user) {
        return this.createOrUpdateTripExtra(firebaseId, null, {
          isValid: false,
          missingData: ["user"],
          validationResults: {
      ...
      console.error("❌ TripExtra validation error:", error);
      return this.createOrUpdateTripExtra(firebaseId, null, {
        isValid: false,
  ...
  async validateBackendFlow(firebaseId, user) {
    ...
    if (!trip) {
      return this.createOrUpdateTripExtra(firebaseId, user.tripId, {
        isValid: false,
        missingData: ["trip"],
        validationResults: {
          user: { 
    ...
    // Store in TripExtra model
    return this.createOrUpdateTripExtra(firebaseId, trip._id, {
      isValid,
      missingData,
      validationResults
    });
  ...
  async validateFrontendFlow(firebaseId, localStorageData) {
    try {
      ...
      // For frontend-only flow, we don't save to TripExtra model since trip doesn't exist in DB yet
      return {
        isValid,
        missingData,
        validationResults,
      ...
      console.error("❌ TripExtra frontend validation error:", error);
      return {
        isValid: false,
        missingData: ["validation_error"],
        validationResults: { error: { message: error.message } },
  ...
  // 💾 Create or update TripExtra model
  async createOrUpdateTripExtra(firebaseId, tripId, validationData) {
    try {
      let tripExtra = await TripExtra.findOne({ userId: firebaseId, tripId });
      
      if (!tripExtra) {
        tripExtra = new TripExtra({
          userId: firebaseId,
          tripId: tripId,
          ...validationData,
        ...
        tripExtra.isValid = validationData.isValid;
        tripExtra.missingData = validationData.missingData;
        tripExtra.validationResults = validationData.validationResults;
        tripExtra.lastValidated = new Date();
      }

      await tripExtra.save();

      return {
        isValid: tripExtra.isValid,
        missingData: tripExtra.missingData,
        validationResults: tripExtra.validationResults,
        summary: tripExtra.getSummary(),
        flow: "backend"
      };

    } catch (error) {
      console.error("❌ TripExtra save error:", error);
      return {
        isValid: false,
        missingData: ["validation_error"],
        validationResults: { error: { message: error.message } },
  ...
  // 🔍 Quick check - just get stored validation from TripExtra model
  async getStoredValidation(firebaseId, tripId) {
    try {
      const tripExtra = await TripExtra.findOne({ userId: firebaseId, tripId });
      ...
      return {
        ...
        validationResults: tripExtra.validationResults,
        summary: tripExtra.getSummary(),
        lastValidated: tripExtra.lastValidated,
        flow: "backend"
      };

    } catch (error) {
      console.error("❌ TripExtra getStoredValidation error:", error);
      return null;
    }
  }
  ...
  async getLocalStorageData(firebaseId) {
    try {
      ...
      console.error("❌ TripExtra getLocalStorageData error:", error);
      return { error: "Failed to get localStorage data" };
    }
  }
}

module.exports = new TripExtraService();
